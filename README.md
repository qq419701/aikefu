...
所有系统设置均支持**后台热更新**，无需重启服务：

| 设置项 | 说明 | 默认值 |
|---|---|---|
| 豆包API Key | 火山方舟API密钥 | — |
| Lite模型名称 | 意图识别/多轮对话模型 | 见.env |
| Pro模型名称 | 退款决策/情绪安抚模型 | 见.env |
| AI最大Token数 | 单次回复最大Token | 500 |
| AI温度参数 | 0=精确，1=创意 | 0.3 |
| 多轮对话保留轮次 | AI记忆的对话轮数 | 10 |
| 会话超时分钟 | 超时后重置上下文 | 30 |
| 知识库相似度阈值 | 知识库匹配精度 | 0.6 |
| 自动回复延迟 | 模拟人工，避免风控 | 1-3秒 |
| 情绪转人工等级 | 触发转人工的情绪级别 | 3（严重） |
| 黑名单退款次数 | 自动加黑名单的退款次数 | 3次/30天 |
